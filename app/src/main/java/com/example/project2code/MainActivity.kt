    ...
    View.OnLongClickListener
{
        ...
        RecyclerView.Adapter<ItemHolder>()
    {
        ...
        override fun onBindViewHolder(holder: ItemHolder, position: Int)
        {
            ...
            holder.txtCreatedDate.text =
                DBHelper.USA_FORMAT.format(item.creationDate)
            holder.txtUpdatedDate.text =
                DBHelper.USA_FORMAT.format(item.updateDate)
    ...
    fun populateRecyclerView()
    {
        ...
        with(cursor) {
            while (moveToNext())
            {
                ...
                val status = getInt(4)
                val item =
                    Item(id, description, createDate!!, updateDate!!, status)
                bucketlist.add(item)
                bucketlist.sort()
    ...
    {
        class MyDialogInterfaceListener(val id: Int) :
        ...
        if (view != null)
        {
            ...
            alertDialogBuilder.setPositiveButton("Yes",
                MyDialogInterfaceListener(name.toInt())
                                                )
